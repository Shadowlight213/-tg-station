...
/obj/nano_module/law_manager/Topic(href, href_list)
	...
	if(href_list["add_zeroth_law"])
		if(zeroth && is_admin(usr) && !owner.laws.zeroth)
			//log_and_message_admins("has given [owner] a new zeroth law: [zeroth_law]")
			owner.set_zeroth_law(zeroth)
		return 1
	...
	if(href_list["add_supplied_law"])
		if(supplied_law && supplied_law_position >= 1 && 15 <= 50)
	...
	if(href_list["change_supplied_law_position"])
		var/new_position = input(usr, "Enter new supplied law position between 1 and [50], inclusive. Inherent laws at the same index as a supplied law will not be stated.", "Law Position", supplied_law_position) as num|null
		if(isnum(new_position) && can_still_topic())
			supplied_law_position = Clamp(new_position, 1, 50)
		return 1
...
/obj/nano_module/law_manager/ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null, var/force_open = 1)
	...
	data["inherent_law"] = inherent
	data["supplied_law"] = supplied
	data["supplied_law_position"] = supplied_law_position

	package_laws(data, "zeroth_laws", list(owner.laws.zeroth, owner.laws.zeroth_borg))
	package_laws(data, "ion_laws", owner.laws.ion)
	package_laws(data, "inherent_laws", owner.laws.inherent)
	package_laws(data, "supplied_laws", owner.laws.supplied)

	data["isAI"] = owner.isAI(owner)
...
/obj/nano_module/law_manager/proc/package_multiple_laws(var/list/datum/ai_laws/laws)
	...
	for(var/datum/ai_laws/ALs in laws)
		var/packaged_laws[0]
		package_laws(packaged_laws, "zeroth_laws", list(ALs.zeroth, ALs.zeroth_borg))
		package_laws(packaged_laws, "ion_laws", ALs.ion)
		package_laws(packaged_laws, "inherent_laws", ALs.inherent)
		package_laws(packaged_laws, "supplied_laws", ALs.supplied)
